...
const debug = require('debug')('node-invoker:content-negotiation');
...
function determineContentTypes(contentType) {
    // TODO correctly handle content-type charset, instead of ignoring it
    contentType = (contentType || 'text/plain').split(';')[0].trim();
    const acceptedContentType = mediaTypeNegotiator(contentType, SUPPORTED_MEDIA_TYPES)[0];
    debug(`Received ${contentType}, accepted ${acceptedContentType}`);
