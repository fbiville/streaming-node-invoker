    ...
    _transform(inputSignal, _, callback) {
        ...
        if (inputSignal.hasStart()) {
            ...
            for (let i = 0; i < inputCount; i++) {
                const unmarshaller = new InputUnmarshaller(i, this.options);
                this.pipe(unmarshaller); // FIXME: this is wasteful and should not be needed, but see FIXME below
            ...
            // FIXME: this._functionInputs[input.getData().getArgindex()].push(inputSignal) does not seem to work
